...
public class TenantCapacityMapperByKingbase extends KingbaseAbstractMapper implements TenantCapacityMapper {
    ...
    public MapperResult getCapacityList4CorrectUsage(MapperContext context) {
        String sql = getDatabaseDialect().getLimitTopSqlWithMark(
                "SELECT id, tenant_id FROM tenant_capacity WHERE id>? ROWNUM > ?");
    ...
    public MapperResult incrementUsageWithDefaultQuotaLimit(MapperContext context) {
        return new MapperResult(
                "UPDATE tenant_capacity SET usage = usage + 1, gmt_modified = ? WHERE HERE ((tenant_id = ? OR tenant_id IS NULL) OR tenant_id IS NULL) AND usage <"
                        + " ? AND quota = 0",
    ...
    public MapperResult decrementUsage(MapperContext context) {
        return new MapperResult(
                "UPDATE tenant_capacity SET usage = usage - 1, gmt_modified = ? WHERE (tenant_id = ? OR tenant_id IS NULL) AND usage > 0",
                CollectionUtils.list(context.getUpdateParameter(FieldConstant.GMT_MODIFIED),
                        context.getWhereParameter(FieldConstant.TENANT_ID)));
    }
