...
public class HistoryConfigInfoMapperBySqlserver extends HistoryConfigInfoMapperByMySql {
    ...
    public MapperResult removeConfigHistory(MapperContext context) {
        String sql = "DELETE FROM his_config_info WHERE id in (SELECT top(?) id FROM his_config_info WHERE gmt_modified < ?) ";
        return new MapperResult(sql,
                CollectionUtils.list(context.getWhereParameter(FieldConstant.LIMIT_SIZE),
